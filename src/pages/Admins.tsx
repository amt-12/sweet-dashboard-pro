...
const Admins = () => {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const res = await fetchWithAuth('http://localhost:5000/api/admins');
  ...
  const loadPermissions = async () => {
    ...
    try {
      const res = await fetchWithAuth('http://localhost:5000/api/permissions');
  ...
  const loadRoles = async () => {
    ...
    try {
      const res = await fetchWithAuth('http://localhost:5000/api/roles');
  ...
  const deletePermission = async (permission: PermissionItem) => {
    ...
    try {
      const res = await fetchWithAuth(`http://localhost:5000/api/permissions/${permission._id}`, { method: 'DELETE' });
  ...
  const remove = async (id: string) => {
    ...
    try {
      const res = await fetchWithAuth(`http://localhost:5000/api/admins/${id}`, { method: 'DELETE' });
  ...
  const savePermissions = async (permissions: string[]) => {
    ...
    try {
      const res = await fetchWithAuth(`http://localhost:5000/api/admins/${selectedAdmin._id}/permissions`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  const assignRole = async (roleData: RoleItem) => {
    ...
    try {
      const res = await fetchWithAuth(`http://localhost:5000/api/admins/${selectedAdmin._id}/role`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
