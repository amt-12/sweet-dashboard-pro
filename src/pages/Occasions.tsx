...
const Occasions = () => {
	...
	useEffect(() => {
		...
		api.occasions
			...
			.catch((err: any) => setError(err?.message || "Failed to load occasions"))
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			if (editingId) {
				const res: any = await api.occasions.update(editingId, payload);
				const t = res && (res._id || res.id) ? { id: res._id || res.id, ...res } : res && res.data ? res.data : res;
				setItems((prev) => prev.map((it) => (String(it.id) === String(editingId) ? { id: t.id, name: t.name, description: t.description } : it)));
			} else {
				const res: any = await api.occasions.create(payload);
				const t = res && (res._id || res.id) ? { id: res._id || res.id, ...res } : res && res.data ? res.data : res;
				setItems((prev) => [{ id: t.id, name: t.name, description: t.description }, ...prev]);
			}
			closeModal();
		} catch (err: any) {
	...
	if (loading) return <div className="p-8 text-center">Loading occasions...</div>;
	...
	return (
		<div className="space-y-8 animate-fade-in font-lora">
			<div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
				<div>
					<h2 className="text-3xl font-bold font-playfair text-[#1A2744]">Occasions <span className="inline-block hover:scale-110 transition-transform">🎉</span></h2>
					<p className="text-[#8D6E63] mt-1">Manage occasions for special orders.</p>
				</div>
			...
			<div className="flex items-center gap-4 bg-white p-4 rounded-2xl shadow-sm border border-[#D4A373]/20">
				<div className="relative flex-1">
					...
					<input type="text" placeholder="Search occasions..." className="w-full pl-11 pr-4 py-3 rounded-xl bg-[#F5ECD7]/30 text-[#1A2744] outline-none focus:ring-2 focus:ring-[#D4A373]/50 transition-all placeholder:text-[#8D6E63]/60" />
				</div>
			</div>

			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
				{items.map((item) => (
					<div key={item.id} className="bg-white p-6 rounded-2xl shadow-sm border border-[#D4A373]/20 hover:shadow-bakery-lg hover:-translate-y-1 transition-all duration-300 group">
						<div className="flex justify-between items-start mb-4">
							<div className="w-14 h-14 rounded-2xl bg-[#F5ECD7]/50 flex items-center justify-center text-[#D4A373] font-playfair font-bold text-2xl border border-[#D4A373]/20 shadow-inner">{String(item.name).charAt(0)}</div>
							<div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
								<button onClick={() => openEdit(item)} className="p-2 bg-[#F5ECD7] rounded-full text-[#8D6E63] hover:text-[#1A2744] hover:bg-[#D4A373] hover:text-white transition-all shadow-sm"><Edit size={16} /></button>
								<button onClick={() => handleDelete(item.id)} className="p-2 bg-red-50 rounded-full text-red-400 hover:text-white hover:bg-red-500 transition-all shadow-sm"><Trash2 size={16} /></button>
							</div>
						</div>
						<h3 className="text-xl font-bold font-playfair text-[#1A2744] mb-2">{item.name}</h3>
						<p className="text-sm text-[#8D6E63] mb-4 line-clamp-2">{item.description}</p>
					</div>
				))}
			</div>
				...
				<div className="fixed inset-0 z-50 flex items-start justify-center pt-24 bg-black/50">
					<div className="w-full max-w-lg bg-white rounded-2xl p-6 shadow-xl mt-6 mx-4 max-h-[calc(100vh-6rem)] overflow-y-auto">
						<div className="flex items-start justify-between">
							<h3 className="font-playfair text-xl font-bold mb-4">{editingId ? "Edit Occasion" : "Add Occasion"}</h3>
