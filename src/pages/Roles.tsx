...
const Roles = () => {
  ...
  const saveRole = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const url = editingRole ? `http://localhost:5000/api/roles/${editingRole._id}` : 'http://localhost:5000/api/roles';
  ...
  const deleteRole = async (role: Role) => {
    ...
    try {
      const res = await fetchWithAuth(`http://localhost:5000/api/roles/${role._id}`, { method: 'DELETE' });
